...
namespace NUnit.Extras
{
	...
	public class TestResultLoader
	{
		...
		{
			public DummyTestCase( string pathName, string testName, int assemblyKey ) 
				: base( pathName, testName, assemblyKey ) { }
		...
		private TestSuiteResult LoadSuite( XmlNode xmlNode )
		{
			...
			result.Executed = result.Test.ShouldRun;
		...
		{
			string testName = LastPartOf( GetTestName( xmlNode ) );
			UITestNode testCase =  new UITestNode( new DummyTestCase( suiteName, testName, assemblyKey ) );
			...
			else
			{
				...
				if ( reasonNode != null )
				{
					...
					result.Test.IgnoreReason = reason;
				}
			}
		...
		private string LastPartOf( string testName )
		{
			if ( testName != null )
			{
				int index = testName.LastIndexOf( '.' );
				if ( index >= 0 ) testName = testName.Substring( index+1 );
			}
